                    ...
                    if (mysqli_stmt_execute($stmt)) {
                        mysqli_stmt_store_result($stmt);
                            if (mysqli_stmt_num_rows($stmt) == 0) {
                                $username_err = "This school does not exist. Please try another";
                            } else {
...
if (empty($username_err) && empty($password_err) && empty($confirm_password_err)) {
    $sql = "UPDATE `school` SET `password` = ? WHERE schoolName = ? ";
        if ($stmt = mysqli_prepare($conn, $sql)) {
      ...
      <h1>School password change</h1>
      <p>Use the form below to change a school password in your system.</p>
