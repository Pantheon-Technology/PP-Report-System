...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE `parents` SET `emergencyName2` = ?, `relation2` = ?, `emergencyPostcode2` = ?, `emergencyContactNum2` = ? WHERE parentUsername = '$username'";
         ...
         mysqli_stmt_bind_param($stmt, "ssss", $param_name, $param_relation, $param_postcode, $param_phoneNum);
            ...
            $param_postcode = trim($_POST["postcode"]);
                ...
                if (mysqli_stmt_execute($stmt)) {
                    echo '<script>alert("' . $username . " " . ' second emergency contact details has been updated successfully")</script>';
                } else {
