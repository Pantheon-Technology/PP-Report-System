...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    // Handle the deletion
    $sql = 'DELETE FROM `schoolStudent` WHERE `studentID` = ?';
...
<?php
$sql = "SELECT * FROM `schoolStudent`";

if (!empty($search)) {
    $sql .= " WHERE `studentName` LIKE '%$search%' OR `studentUsername` LIKE '%$search%'";
}
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {   
        ...
        echo "<p><b>Student: " . htmlspecialchars($row['studentName']) . "</b></p>";
        echo "<p><b>School: " . htmlspecialchars($row['schoolName']) . "</b></p>";
        ...
        echo "<button type='submit' class='w3-button w3-red w3-round w3-margin-bottom w3-hover-opacity'>Delete this student</button>";
        echo "</form>";
        echo "</div>";
        echo "</div>";
    }
